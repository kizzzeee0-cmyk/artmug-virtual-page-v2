...
function renderAvatars(list){
  const avatars=(list||[]).filter(a=>a && (a.name || (a.images||[]).length));
  const photos=[];
  avatars.forEach(a=>(a.images||[]).filter(Boolean).forEach((u,i)=>photos.push({url:u,alt:`${a.name||'아바타'} ${i+1}`})));
  const names=avatars.filter(a=>String(a.name||'').trim()).map(a=>`<span class="avatar-name-pill">${esc(a.name)}</span>`).join('');
  const photoWall=photos.length
    ? `<div class="avatar-wall">${photos.map(x=>`<button type="button" class="avatar-tile" aria-label="${esc(x.alt)} 크게 보기"><img src="${esc(x.url)}" alt="${esc(x.alt)}" loading="lazy" decoding="async" data-zoom="true"></button>`).join('')}</div>`
    : '<div class="empty">등록된 아바타 사진이 없습니다.</div>';
  return `<section class="section">${sectionHead('OWNED AVATARS','보유 아바타','현재 작업 가능한 보유 아바타 목록입니다.')}<div class="avatar-name-row">${names||'<span class="avatar-name-empty">등록된 아바타 이름이 없습니다.</span>'}</div><p class="avatar-gift-note">목록에 없는 아바타는 기프트(선물)로 보내주셔야 작업이 가능합니다.</p>${photoWall}</section>`;
}
...
function portfolioPanel(cat){
  ...
  const cards=cat.layout==='compare'
    ? items.map((it,i)=>`<article class="portfolio-item compare-card"><div class="compare-pair"><div class="compare-media"><span class="compare-label">BEFORE</span>${img(it.before,`${cat.name} before ${i+1}`,'portfolio-zoom')}</div><div class="compare-media"><span class="compare-label">AFTER</span>${img(it.after,`${cat.name} after ${i+1}`,'portfolio-zoom')}</div></div>${cat.showCaption!==false&&it.caption?`<div class="portfolio-caption">${esc(it.caption)}</div>`:''}</article>`).join('')
    : items.map((it,i)=>`<article class="portfolio-item"><div class="portfolio-media ${kind==='wide'?'portfolio-media-wide':''}" style="aspect-ratio:${esc(cat.ratio||(kind==='wide'?'16 / 9':'1 / 1'))}">${img(it.media,`${cat.name} ${i+1}`,'portfolio-zoom')}</div>${cat.showCaption!==false&&it.caption?`<div class="portfolio-caption">${esc(it.caption)}</div>`:''}</article>`).join('');
