...
- 협업 작가 이미지는 고정 높이 영역을 사용하지 않고 업로드한 원본 비율대로 세로 길이가 자연스럽게 늘어납니다.
- 포트폴리오 화살표를 제거하고 아래 드래그 가능한 슬라이드 바로 좌우 이동합니다.
- 보유 아바타 카테고리명은 위쪽, 사진은 아래쪽에 가로형으로 정렬하며 `N장` 표시를 제거했습니다.
- 기존 이미지 클릭 확대, 성형 1:1 contain, 최신 포트폴리오 우선 정렬은 유지됩니다.

## v2.3 화면 표시 변경

- 보유 아바타: 탭 전환 없이 아바타 이름별 그룹으로 모든 사진을 한 화면에 표시
- 아바타 썸네일 크기 축소 및 원본이 잘리지 않도록 contain 표시
- 성형 비교: Before / After 각각 독립적인 1:1 정사각형으로 표시하며 이미지 잘림 없음
- 포트폴리오 이미지 클릭 시 전체 화면 확대 보기 지원
- 페이셜 / 뽀잉눈: 작은 카드형 가로 슬라이드
- 닐로툰: 더 큰 16:9 카드형 가로 슬라이드
- 포트폴리오가 많아지면 좌우 화살표 또는 가로 스크롤로 이동
- 새 포트폴리오 업로드 시 updatedAt을 기록하고 최신 작업물을 먼저 표시
- 협업 작가 이미지: 업로드 원본 비율을 유지하고 잘리지 않도록 표시
